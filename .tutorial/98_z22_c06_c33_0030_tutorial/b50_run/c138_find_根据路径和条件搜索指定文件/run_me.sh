...
l32_call_default_cs_club_learning_grade_entry_shell_script(){
    ...
    # NOTE 缺省的_run_me_入口
    cs_club_tutorail_learning_run_me_entry_script_path=${cs_club_tutorail_path}/cs36_learning_grade/ab50_20_run_me_entry_script.sh

    # echo "l32_28_缺省的grade评分脚本路径---> ${cs_club_tutorail_learning_run_me_entry_script_path}"
    ...
    if [[ -f ${cs_club_tutorail_learning_run_me_entry_script_path} ]]; then
        chmod +x ${cs_club_tutorail_learning_run_me_entry_script_path}
        ...
        # echo "source ${cs_club_tutorail_learning_run_me_entry_script_path} ${current_script_full_path}"
        # echo -e "\n"

        # pause_60_second

        # echo "准备_执行_run_me_的_入口_脚本_ab50_20_run_me_entry_script.sh"
        source ${cs_club_tutorail_learning_run_me_entry_script_path} ${current_script_full_path}
        # echo "执行_完毕_run_me_的_入口_脚本_ab50_20_run_me_entry_script.sh"
    else
        echo "没有找到_run_me_的_入口_脚本_ab50_20_run_me_entry_script.sh"
        echo "请在_cloudstudio_的_club课程_tutorial目录_下_创建_ab50_20_run_me_entry_script.sh"
        echo "然后_再_运行_这个_脚本"
        echo -e "\n"
    fi
