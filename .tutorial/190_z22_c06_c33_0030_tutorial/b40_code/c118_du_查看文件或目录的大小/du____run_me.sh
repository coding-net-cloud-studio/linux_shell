...
clear && echo -e "\n\n" # NOTE 清理一下屏幕内容
###NOTE_5_在下方_编写你的_练习_代码_你的代码从下面这里开始_替换前
...
# 下面这个变量的几种状态
# 还没有执行 执行成功 执行失败 文件不存在

export vd46_22_exectue_student_file_status="还没有执行"



execute_student_exercise_file(){

    # NOTE 获取_本_脚本的完整路径
    local current_script_full_path=$(realpath "$0")

    local run_me_file_name=$(basename $current_script_full_path)

    local directory=$(dirname $current_script_full_path )

    local student_file_name=$(echo $run_me_file_name | awk -F'____run_me' '{print $1$2}' )

    local student_file_path=${directory}/${student_file_name}

    echo "l32_28_当前执行的_学生文件_路径---> ${student_file_path}"

    # NOTE 执行_学生文件
    if [[ -f ${student_file_path} ]]; then
        echo "l32_36_执行_学生文件_${student_file_path}"
        chmod +x ${student_file_path}
        ...
        source ${student_file_path}

        # NOTE 执行_学生文件_的_返回值_
        local student_file_return_value=$?

        # echo "l32_44_执行_学生文件_的_返回值_---> ${student_file_return_value}"

        # NOTE 执行_学生文件_的_返回值_
        if [[ ${student_file_return_value} -eq 0 ]]; then
            echo "l32_52_执行_学生文件_的_返回值_---> ${student_file_return_value}"
            export vd46_22_exectue_student_file_status="执行成功"
        else
            echo "l32_60_执行_学生文件_的_返回值_---> ${student_file_return_value}"
            export vd46_22_exectue_student_file_status="执行失败"
        fi

    else
        echo "l32_68_执行_学生文件_的_返回值_---> ${student_file_return_value}"
        export vd46_22_exectue_student_file_status="文件不存在"
    fi

    echo "执行 ${student_file_path} 结果如下"
    echo ${vd46_22_exectue_student_file_status}
...
l32_call_default_cs_club_learning_grade_entry_shell_script(){
    ...
    if [[ -f ${cs_club_tutorail_learning_grade_script_path} ]]; then
        ...
        source ${cs_club_tutorail_learning_grade_script_path} ${current_script_full_path} ${vd46_22_exectue_student_file_status}
...
grade() {
    ...
    echo -e "运行在****$(pwd)/${0}脚本的****${FUNCNAME}()函数中****第${LINENO}行\n"
